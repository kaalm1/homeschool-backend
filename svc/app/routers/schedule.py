...
from svc.app.utils.security import get_current_user, get_db
...
def today(user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    kid_ids = [
        k.id
        for k in db.execute(select(Kid).where(Kid.parent_id == user.id)).scalars().all()
    ]
    rows = (
        db.execute(select(Activity).where(Activity.kid_id.in_(kid_ids))).scalars().all()
    )
    return rows
...
def add_activity(
    data: ActivityIn, user=Depends(get_current_user), db: Session = Depends(get_db)
):
...
def toggle(
    data: ToggleIn, user=Depends(get_current_user), db: Session = Depends(get_db)
):
